...
setup_script_rendered = setup_script_template.render(release_tag=release_tag)
setup_script_path.write_text(setup_script_rendered + "\n")

checksums["setup_macvlan.sh"] = hashlib.sha256(
    setup_script_path.read_bytes()
).hexdigest()


checksums_rendered = checksums_template.render(checksums=checksums)
checksums_path.write_text(checksums_rendered + "\n")

release_notes_rendered = release_template.render(
    release_tag=release_tag, devices=devices, checksums=checksums
)
