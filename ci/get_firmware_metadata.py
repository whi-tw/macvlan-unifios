...
def check_attr_in_firmware_metadata(device, firmware_metadata, attr):
    if attr not in firmware_metadata or not firmware_metadata[attr]:
        raise Exception(f"Firmware {firmware_version} for device {device} has no {attr} specified in firmware_metadata.json")
...
with open(os.path.join(firmware_dir_path, 'firmware_metadata.json')) as f:
    ...
    firmware_metadata['kernel-url-base64'] = base64.b64encode(firmware_metadata['kernel-url'].encode('utf-8')).decode('utf-8')
